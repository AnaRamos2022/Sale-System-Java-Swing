...
public class ArregloVentas {
	...
	public int obtenerCodigoCorrelativo() {
		...
		if (listadoVenta.size() == 0) {
			...
			Venta utlimaVentaRegistrada = listadoVenta.get(ultimaPosicionArrayList);
		...
		private void archivarVentas() {
			...
			try {
				...
				for(int i=0; i<listadoVenta.size(); i++) {
					v=listadoVenta.get(i);
					fila=v.getCodigoVenta() + ";" + v.getCodigoCliente() + ";" + v.getCodigoProducto() + ";" + v.getCantidad() + ";" + v.getPrecio() + ";" + v.getFecha();
					pw.println(fila);
				}
				pw.close();
			}
